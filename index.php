<!doctype html>
<html class="no-js" lang="fr">
    <?php include './partials/head.php' ?>
    <body>
        <?php $active = 'accueil' ?>
        <?php include './partials/header.php' ?>
        <main>
            <section>
                <div class="container section-container">
                    <div class="row align-items-stretch justify-content-between">
                        <div class="service-block">
                            <div class="service-body">
                                <div class="service-title">
                                    <div class="circled-icon"><i class="fas fa-home"></i></div>
                                    <h3>Entretien du domicile</h3>
                                </div>
                                <div class="service-details">
                                    <ul class="service-details-list">
                                        <li>Ménage régulier</li>
                                        <li>Ménage ponctuel</li>
                                    </ul>
                                </div>
                                <div class="service-description">
                                    Aérer, dépoussiérer, aspirer, laver et traiter les sols, nettoyer miroirs et vitres, entretien des sanitaires et salle d’eau, faire les lits, utilisation du lave-linge/sèche-linge/lave-vaisselle, soin du linge, repassage, vider les poubelles, nettoyer l’intérieur des électroménagers
                                </div>
                            </div>
                            <div class="service-footer">
                                <div class="service-buttons">
                                    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#quoteModal" data-prestation="entretien du domicile">Demande de renseignements</button>
                                </div>
                                <div class="service-price">
                                    <span class="service-price-small">A partir de</span>
                                    <span class="service-price-price">9€/h*</span>
                                    <span class="service-price-asterisk">*après avantage fiscal</span>
                                </div>
                            </div>
                        </div>

                        <div class="service-block">
                            <div class="service-body">
                                <div class="service-title">
                                    <div class="circled-icon"><i class="fas fa-child"></i></div>
                                    <h3>Garde d'enfants</h3>
                                </div>
                                <div class="service-details">
                                    <ul class="service-details-list">
                                        <li>Garde d'enfants</li>
                                        <li>Accompagnement périscolaire</li>
                                    </ul>
                                </div>
                                <div class="service-description">
                                    Garde d'enfants à votre domicile, sorties d'école, mercredis, samedis, imprévus
                                </div>
                            </div>
                            <div class="service-footer">
                                <div class="service-buttons">
                                    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#quoteModal" data-prestation="garde d'enfants">Demande de renseignements</button>
                                </div>
                                <div class="service-price">
                                    <span class="service-price-small">A partir de</span>
                                    <span class="service-price-price">10€/h*</span>
                                    <span class="service-price-asterisk">*après avantage fiscal</span>
                                </div>
                            </div>
                        </div>

                        <div class="service-block">
                            <div class="service-body">
                                <div class="service-title">
                                    <div class="circled-icon"><i class="fas fa-utensils"></i></div>
                                    <h3>Préparation de repas</h3>
                                </div>
                                <div class="service-details">
                                    <ul class="service-details-list">
                                        <li>Aide à la préparation des repas</li>
                                        <li>Préparation de "lunch box"</li>
                                    </ul>
                                </div>
                                <div class="service-description">
                                    Eplucher et couper les légumes, (pré)cuisson d'aliments, préparation de «lunch box»
                                </div>
                            </div>
                            <div class="service-footer">
                                <div class="service-buttons">
                                    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#quoteModal" data-prestation="préparation de repas">Demande de renseignements</button>
                                </div>
                                <div class="service-price">
                                    <span class="service-price-small">A partir de</span>
                                    <span class="service-price-price">10€/h*</span>
                                    <span class="service-price-asterisk">*après avantage fiscal</span>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>

            <section>
                <div class="container section-container">
                    <div class="row">
                        <div class="section-title col-12">
                            <h2 class="section-title-title">Zone d'intervention</h2>
                            <div class="section-title-soft">Billère, Bizanos, Gan, Gelos, Jurançon, Mazères-Lezons, Rontignon et Uzos</div>
                        </div>
                        <div class="section-content intervention-map col-12">
                        </div>
                    </div>
                </div>
            </section>

            <section>
                <div class="container section-container">
                    <div class="row">
                        <div class="section-title col-12">
                            <h2 class="section-title-title">Avantages</h2>
                            <div class="section-title-soft">Pourquoi choisir Aux Petits Soins Chez Vous ?</div>
                        </div>
                        <div class="col-6 col-sm-4 col-lg-2 advantage">
                            <div class="circled-icon circled-icon-shadowed"><i class="far fa-building"></i></div>
                            <div class="advantage-text">Entreprise inscrite au registre de la <a href="http://www.cma64.fr/">CMA</a></div>
                        </div>
                        <div class="col-6 col-sm-4 col-lg-2 advantage">
                            <div class="circled-icon circled-icon-shadowed"><i class="fas fa-percent"></i></div>
                            <div class="advantage-text">Avantage fiscal (reduction ou crédit d'impôt)</div>
                        </div>
                        <div class="col-6 col-sm-4 col-lg-2 advantage">
                            <div class="circled-icon circled-icon-shadowed"><i class="far fa-handshake"></i></div>
                            <div class="advantage-text">Assurance casse comprise</div>
                        </div>
                        <div class="col-6 col-sm-4 col-lg-2 advantage">
                            <div class="circled-icon circled-icon-shadowed"><i class="fas fa-euro-sign"></i></div>
                            <div class="advantage-text">Aucun frais de gestion ou de dossier</div>
                        </div>
                        <div class="col-6 col-sm-4 col-lg-2 advantage">
                            <div class="circled-icon circled-icon-shadowed"><i class="far fa-file-alt"></i></div>
                            <div class="advantage-text">Devis et analyse de vos besoins gratuits</div>
                        </div>
                        <div class="col-6 col-sm-4 col-lg-2 advantage">
                            <div class="circled-icon circled-icon-shadowed"><i class="far fa-thumbs-up"></i></div>
                            <div class="advantage-text">Pas de charges patronales</div>
                        </div>
                    </div>
                </div>
            </section>
        </main>
        <?php include './partials/footer.php' ?>
    </body>
</html>
